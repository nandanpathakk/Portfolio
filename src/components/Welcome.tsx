    ...
    const playTypeSound = () => {
        const audio = new Audio('/sounds/soft-type.mp3');
        audio.volume = 0.05;
        audio.play().catch(e => console.log("Audio play failed:", e));
    };

    // Sound effect for bridge formation
    const playBridgeSound = () => {
        const audio = new Audio('/sounds/bridge-form.mp3');
        audio.volume = 0.3;
        audio.play().catch(e => console.log("Audio play failed:", e));
    };
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            setShowPulse(true);
            // Play bridge sound when the pulse starts
            playBridgeSound();
            
            setTimeout(() => {
            setShowBridges(true);
            }, 800); // Start bridges after the pulse effect
        }, 800);
        return;
        }
        ...
        if (charIndex < currentSentence.length) {
            ...
            // Play typing sound at reduced frequency
            if (charIndex % 3 === 0) {
            playTypeSound();
            }
        } else {
            ...
            }, 1800); // Slightly longer pause between sentences
        }
    ...
    useEffect(() => {
        ...
        const audio = new Audio('/sounds/dimension-shift.mp3');
        audio.volume = 0.4;
        setTimeout(() => {
            audio.play().catch(e => console.log("Audio play failed:", e));
        }, 800);
